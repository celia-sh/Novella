...
### Mistake 4: Treating An Unversioned Server Echo As Newer State

**Bad**: A local mutation receives an acknowledgement, is marked synchronized,
then an immediately refetched server payload overwrites it merely because it
came from the server. Read replicas, response caches, and independently loaded
screens can still return the pre-mutation value.

**Good**: Publish mutations to one local projection synchronously and preserve
pending/acknowledged causality with a revision when available. If the server
exposes no revision, keep the current-process value only until the server
returns the same canonical value, then clear the barrier immediately. Do not
invent a time-based authority lease. Every mounted consumer must subscribe to
that same projection.

For navigation-driven state such as reading progress, also verify the complete
round trip:

```
visible item → local projection → detail display → route chapter → restore
             ↘ durable cache → serialized server write ↗
```

A successful endpoint unit test does not verify this loop. Test focus return,
immediate re-entry, stale server echo, and exact local restoration separately.
...
- [ ] Defined `null`, missing, and empty-string semantics separately for every
